...
Cv2Pdf.prototype.convert = function (callback) {
  ...
  if (this.htmlFile) {
    this.writeHtml(html, this.htmlFile, callback);
  } else {
...
Cv2Pdf.prototype.watch = function (callback) {
  var context = this;
  this.watcher = watcher(this.markdownFile, function (err) {
    if (err) {
      return callback(err);
    }
    context.convert(callback);
  });
};


Cv2Pdf.prototype.unwatch = function () {
  if (this.watcher) {
    this.watcher.close();
  }
};
...
Cv2Pdf.prototype.writeHtml = function (html, htmlFile, callback) {
